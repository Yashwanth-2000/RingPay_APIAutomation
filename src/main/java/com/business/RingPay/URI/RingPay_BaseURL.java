package com.business.RingPay.URI;

public class RingPay_BaseURL {
	
public static String testingServiceURL= "https://testing-gateway.test.paywithring.com";
public static String userGatewayURL= "https://user-gateway.test.ideopay.in";
public static String txnGatewayURL= "https://txn-gateway.test.ideopay.in";
public static String bigDataPythonURL= "https://py-data.test.ideopay.in";
public static String bigDataURL= "https://data.test.ideopay.in";

		
}
